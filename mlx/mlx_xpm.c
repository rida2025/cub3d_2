...
void	*mlx_int_parse_xpm(mlx_ptr_t *xvar,void *info,int info_size,char *(*f)())
{
  ...
  if (cpp<=2)
    {
      method = 1;
      if (!(colors_direct = ft_malloc((cpp==2?65536:256)*sizeof(int))))
	RETURN;
    }
  else
    if (!(colors = ft_malloc(nc*sizeof(*colors))))
      RETURN;
  ...
  while (i--)
	...
	{
	  if (!(clip_data = ft_malloc(4*width*height)) ||   // ok, nice size ..
